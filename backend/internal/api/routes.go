...
func Routes(db *sql.DB) *http.ServeMux {
	...
	Createarticle := ratelimiter.CreateArticleLimiter.RateMiddleware(http.HandlerFunc(handler.HandelCreateArticle), 10, 2*time.Second)
	...
	// notification
	http.HandleFunc("/api/GetNotification/{page}", handler.HandleGetNotification)
