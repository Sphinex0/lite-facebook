...
func Routes(db *sql.DB) *http.ServeMux {
	...
	// articls
	// posts := ratelimiter.CreateArticleLimiter.RateMiddleware(http.HandlerFunc(handler.HandelGetPosts), 20, 2*time.Second)
	...
	// Createarticle := ratelimiter.CreateArticleLimiter.RateMiddleware(http.HandlerFunc(handler.HandelCreateArticle), 1, 10*time.Second)
	...
	mux.HandleFunc("/api/Event/options/choise", handler.GetEventChoice)
